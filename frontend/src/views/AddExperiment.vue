<template>
  <div class="min-h-screen bg-gray-50">
    <div class="max-w-3xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
      <div class="mb-6">
        <h1 class="text-3xl font-bold text-gray-900 mb-2">Add New Experiment</h1>
        <p class="text-gray-600">Create a new HPC experiment entry</p>
      </div>
        ...
        <form @submit.prevent="handleSubmit" class="space-y-6">
          ...
          <div>
            ...
            <div class="flex gap-2">
              <Input id="github_url" v-model="formData.github_url" placeholder="https://github.com/owner/repo"
                class="flex-1" :class="{ 'border-red-500': errors.github_url }" />
          ...
          <div>
            ...
            <Input id="date" v-model="formData.date" type="date" :class="{ 'border-red-500': errors.date }" />
          ...
          <div>
            ...
            <div class="mb-4 flex gap-2">
              <Input v-model="newTagName" placeholder="Enter new tag name" class="flex-1" @keyup.enter="handleAddTag" />
              <Button type="button" variant="outline" @click="handleAddTag" :disabled="!newTagName.trim() || addingTag">
          ...
          <div>
            ...
            <MarkdownEditor v-model="formData.content" placeholder="Enter markdown content for your experiment..."
              :rows="15" :error="errors.content" />
...
const fetchTags = async () => {
  ...
  try {
    ...
    // Tags are optional, so we don't show an error
    availableTags.value = []
  } finally {
...
const handleSubmit = async () => {
  ...
  try {
    const payload = {
      ...
      tag_ids: formData.value.tags, // Use tag_ids for writing
      github_url: formData.value.github_url.trim() || null,
      content: formData.value.content.trim() || null, // Markdown content
    }

    const response = await experimentsApi.addExperiment(payload)

    toastStore.success('Experiment created successfully!')
    ...
    toastStore.error('Failed to create experiment')
  } finally {
